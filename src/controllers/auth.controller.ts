...
export const signupController = async (
  ...
  try {
    ...
    logger.error(`signupController AuthController Error: ${error.message}`);
...
export const loginController = async (
  ...
  await updateUser(user._id, { refreshToken }, next);
...
export const logoutController = async (
  ...
  req.session.destroy(err => {
    if (err) {
      logger.error(`logoutController AuthController Error: ${err.message}`);
      return next(new (CustomException as any)(500, 'Operation unsuccessful'));
    }
    return res.status(200).json({
      status: 'success',
